...
namespace MVCTests
{
    ...
    public class UserMessagingServiceTests
    {
        ...
        [TestMethod()]
        public void GetHelloMsgFromDBTest()
        {
            Mock<IDababaseConnector> mockDabaseConnector = new Mock<IDababaseConnector>();
            mockDabaseConnector.Setup(m => m.HelloMsg(It.IsAny<string>())).Returns("Common name");
            userMessagingService = new UserMessagingService(mockDabaseConnector.Object);

            string msg = userMessagingService.GetHelloMsg("rajee");
